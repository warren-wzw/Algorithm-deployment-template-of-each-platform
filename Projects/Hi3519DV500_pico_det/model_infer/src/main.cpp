...
const char *IMAGE="../data/DBnet_test.jpg";
...
void drawText(cv::Mat &Text_image,cv::Point &text_p,string text)
{
    // 选择字体、字体大小和颜色
    cv::Scalar font_color(255, 0, 0);  // 文本颜色，通常为BGR格式
    int font = cv::FONT_HERSHEY_SIMPLEX;
    double font_scale = 1;
    int font_thickness = 1;
    // 添加文本
    cv::putText(Text_image,text, text_p, font, font_scale, font_color, font_thickness);
}
 
void file_Name_extract(std::vector<string> &file_name,std::vector<string> &file_pure_name,const char *filePath)
{
    struct dirent* entry;
    int image_num=0;
    std::vector<std::string> image_filenames; // 用于保存图片文件名的容器
    std::string ext_name;
    DIR* dir = opendir(filePath);
    if (!dir) {
        std::cerr << "无法打开目录: " << filePath << std::endl;
    }
    while ((entry = readdir(dir))) {
        if (entry->d_type == DT_REG) { // 如果是普通文件
                std::string entry_name = entry->d_name;
                image_filenames.push_back(entry_name);
                image_num++; 
        }
    }
    closedir(dir);
    for(auto image_name :image_filenames){
        size_t dotPos = image_name.find_last_of(".");
        if (dotPos != std::string::npos && dotPos < image_name.length() - 1) {
            ext_name = image_name.substr(dotPos + 1);
        } else {
            cout<<"wrong!!!";
            continue;
        }
        if(ext_name=="jpg"||ext_name=="jpeg"||ext_name=="png"){
            file_name.push_back(image_name);
            // 找到最后一个斜杠或反斜杠的位置
            size_t slashPos = image_name.find_last_of("/\\");
            // 找到最后一个点的位置
            size_t dotPos = image_name.find_last_of(".");
            // 如果找到了点并且点位于斜杠之后，提取文件名（不包含扩展名）
            if (dotPos != std::string::npos && (slashPos == std::string::npos || dotPos > slashPos)) {
                file_pure_name.push_back(image_name.substr(slashPos + 1, dotPos - slashPos - 1)) ;
            } else {
                cout<<"wrong!!!";
                continue;
            }
...
{
    std::vector<string> file_pure_names;
    std::vector<string> &file_pure_name_=file_pure_names;
    std::vector<string> file_names;
    std::vector<string> &file_name_=file_names;
    svp_acl_error ret;
    const char *MODEL_PATH = argv[1];
    const char *IMAGE_FOLDER_PATH = argv[2];
    if (argc != 3){
        printf("Usage: ./xxx  model_path  pic_file_path");
    }
    file_Name_extract(file_name_,file_pure_name_,IMAGE_FOLDER_PATH);
/***************************************************/
/*****************Init ACL**************************/
/***************************************************/
    cout<<"->ACL INIT "<<endl;
...
/***************************************************/
    ...
    for(int num=0;num<(int)input_num;num++){
        ...
        if(num==0){
            input_weight=dims.dims[3];
            input_height=dims.dims[2];
        }
    }
/***************************************************/
...
/***************************************************/
    ...
    for(size_t i=0;i<file_names.size();i++){
        image_name=IMAGE_FOLDER_PATH+file_names[i];
        cout<<"--the process image name is "<<image_name<<endl;
        /******************preprocess********/
        image=imread(image_name,cv::IMREAD_COLOR);
        if (image.empty()) {
            std::cerr << "can't read image." << std::endl;
            return -1;
        }
        image.copyTo(srcimg);
        cout<<"height is "<<input_height<<" weight is "<<input_weight<<endl;
        cv::resize(srcimg, resize_img, cv::Size(input_weight, input_height));
        ...
        std::copy(input.begin(), input.end(), inData); //load data
        ...
        printf("->inference use time %.2f ms\n", eclipes_time/1000.f); 
        /***************************************************/
        /*******************get out put data*************/
        /***************************************************/
    ...
    /******************post process*********************/
    /***************************************************/   
        std::vector<PaddleOCR::StructurePredictResult> result; 
        std::vector<PaddleOCR::StructurePredictResult> &result_ref = result; 
        std::vector<int> bbox_num;
        PaddleOCR::PicodetPostProcessor post_processor_;
        int reg_max = 0;
        post_processor_.init("./dict.txt",score_threshold,nms_threshold); //score nms
        ...
        for(size_t i=0;i<result_ref.size();i++){
            box_point.clear();
            cout<<"->"<<"type is "<<result_ref[i].type<<" score is "<<result_ref[i].confidence<<result_ref[i].box[0]<<result_ref[i].box[1]<<result_ref[i].box[2]<<result_ref[i].box[3]<<endl;
            box_point.push_back(cv::Point(result_ref[i].box[2], result_ref[i].box[1]));  // 右上角
            box_point.push_back(cv::Point(result_ref[i].box[0], result_ref[i].box[1]));  // 左上角
            ...
            text_Point=(cv::Point(result_ref[i].box[0], result_ref[i].box[1]-10));
            text_Value=result_ref[i].type; 
            text_Value+=std::to_string(result_ref[i].confidence);
            drawText(image,text_Point,text_Value);
        }
        INFO_LOG("->save out image!");
        std::string out_name="../post_process_image/"+file_pure_names[i]+"_out.jpg";
