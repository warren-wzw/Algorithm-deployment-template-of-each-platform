...
void PicodetPostProcessor::Run(std::vector<StructurePredictResult> &results,std::vector<std::vector<float>> outs,std::vector<int> ori_shape,std::vector<int> resize_shape, int reg_max) {
  ...
  std::cout<<" in_h "<<in_h<<" in_w "<<in_w<<" scale_factor_h "<<scale_factor_h<<" scale_factor_w "<<scale_factor_w<<std::endl;

  std::vector<std::vector<StructurePredictResult>> bbox_results;
  bbox_results.resize(this->num_class_);
  int num=0;
  std::cout<<"num_class_ is "<<this->num_class_<<std::endl;
  for (size_t i = 0; i < this->fpn_stride_.size(); ++i) {
    ...
    std::cout<<"feature_h "<<feature_h<<" feature_w "<<feature_w<<"this->num_class_"<<this->num_class_<<std::endl;
    ...
    for (int idx = 0; idx < feature_h * feature_w; idx++) {
      ...
      if (score > this->score_threshold_) {
          ...
          //std::cout<<"num is "<<num<<" this->fpn_stride_[i] is "<<this->fpn_stride_[i]<<" reg max is " \
          //<<reg_max<<" score is "<<score<<" feature_w "<<feature_w<<" row "<<row<<" col "<<col<<" bbox_pred size "<<bbox_pred.size()<<" cur_label is "<<cur_label<<std::endl;         
          num++;
  ...
  for (size_t i = 0; i < bbox_results.size(); i++) {
    ...
    std::cout<<"bbox_results[i] size is "<<bbox_results.size()<<" "<<bbox_results[i][0].type<<" confidence is "<<bbox_results[i][0].confidence<<std::endl;
    for (auto box : bbox_results[i]) {
      ...
      //std::cout<<"bbox_results.size() is "<<box.box[0]<<" "<<box.box[1]<<" "<<box.box[2]<<" "<<box.box[3]<<std::endl;
